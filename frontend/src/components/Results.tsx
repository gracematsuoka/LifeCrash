...
const Results: React.FC<ResultsProps> = ({
  ...
  useEffect(() => {
    console.log(
      "Results component received prediction:",
      prediction
    );
    if (prediction.stepsToPrevent) {
      console.log(
        "Prevention steps length:",
        prediction.stepsToPrevent.length
      );
      console.log(
        "Prevention steps content:",
        prediction.stepsToPrevent
      );
    } else {
      console.log(
        "No prevention steps found in prediction object"
      );
    }
  }, [prediction]);
  ...
  const formatSteps = (
    ...
    lines.forEach((line, index) => {
      if (line.match(/^\d+\./) || line.match(/^-\s/)) {
        ...
        const headerLevel =
          line.match(/^(#{1,6})\s/)?.[1].length || 3;
        const text = line.replace(/^#{1,6}\s/, "");
        formattedSteps.push(
          React.createElement(
            `h${headerLevel}`,
            { key: index, className: "prevention-header" },
            text
          )
        );
      } else {
      ...
      <div className="results-card">
        ...
        <div className="results-data">
          ...
          <div className="data-item">
            ...
            <span className="crisis-type">
              {formatCrisisType(prediction.type)}
            </span>
          </div>
          ...
          <div className="data-item prevention-analysis">
            ...
            <div className="analysis-disclaimer">
              Analysis based on your inputs and model
              predictions. Take action now to avoid your
              predicted crisis.
            </div>
          </div>
        </div>
        ...
        <div className="start-over">
          <p className="start-over-text">
            Not satisfied with your prediction?
          </p>
          <button
